...
pg-to-zod --database mydb --views --output schema.ts

# Include functions/procedures (SECURITY DEFINER only by default)
pg-to-zod --database mydb --routines --output schema.ts

# Include all functions including SECURITY INVOKER
pg-to-zod --database mydb --routines --security-invoker --output schema.ts
