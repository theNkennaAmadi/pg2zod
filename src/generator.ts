...
function generateTableSchema(
  ...
  if (options.generateInputSchemas !== false) {
    // Generate INSERT schema (for creating new records)
    insertCode = '';
    ...
    insertCode += `export const ${insertSchemaName} = z.object({\n`;
    ...
    for (const column of table.columns) {
      ...
      // Start with the base type, respecting original nullability
      let zodType = mapColumnToZod(column, metadata, options, warnings);
      ...
      // Make optional ONLY if:
      // 1. Auto-generated (SERIAL, gen_random_uuid())
      // 2. Has a DEFAULT value in the database
      ...
      if (options.includeComments) {
        ...
        insertCode += `  /** ${commentParts.join(', ')} */\n`;
      }
      
      insertCode += `  ${fieldName}: ${zodType},\n`;
    }

    insertCode += `});\n`;
    insertCode += `export type ${insertTypeName} = z.infer<typeof ${insertSchemaName}>;\n`;
    
    // Generate UPDATE schema (for updating existing records - all fields optional)
    updateCode = '';
    ...
    updateCode += `export const ${updateSchemaName} = z.object({\n`;

    for (const column of table.columns) {
      const fieldName = options.useCamelCase ? toCamelCase(column.columnName) : column.columnName;
      const isSerial = column.columnDefault?.includes('nextval') ?? false;
      
      // Skip auto-generated fields (like SERIAL IDs) in update schema
      if (isSerial) {
        continue;
      }
      
      // Get the base type with proper validation
      let zodType = mapColumnToZod(column, metadata, options, warnings);
      ...
      // But preserve the underlying validation when the field IS provided
      zodType = `${zodType}.optional()`;

      if (options.includeComments) {
        updateCode += `  /** ${column.dataType} */\n`;
      }
      
      updateCode += `  ${fieldName}: ${zodType},\n`;
    }
...
export function formatOutput(result: GenerationResult): string {
  let output = `// Generated by pg-to-zod\n`;
